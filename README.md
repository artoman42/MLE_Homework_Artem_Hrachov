...
## Data:
Data is the cornerstone of any Machine Learning project. For uploading the data, use the script located at `data_process/data_generation.py`. The generated data is used to train the model and to test the inference. 
In my homework, it is also stored in repository, according to task conditions. 
## Training:
...
```
Replace `<container_id>` with your running Docker container ID and `<result_name>.csv` with your results's name.
...
## Wrap Up
In conclusion, i have made my first project, that includes basic skills in MLEngineering.